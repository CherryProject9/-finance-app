...
async function saveData() {
    ...
    // Always save to localStorage as a primary/fallback backup
...
async function loadData() {
    ...
    // 1. Try to fetch from server with strict timeout
    try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 10000); // 10s timeout for mobile
        
        console.log('[FinanceOS] Fetching from: ' + API_URL);
        if (window.updateSyncDebug) window.updateSyncDebug('Fetching ' + API_URL);
        const res = await fetch(API_URL, { signal: controller.signal });
        clearTimeout(timeoutId);
        
        if (res.ok) {
            remoteData = await res.json();
            console.log('[FinanceOS] Remote data:', remoteData);
            if (window.updateSyncDebug) {
                const txCount = (remoteData.transactionsState || []).length;
                const budgetCount = Object.keys(remoteData.monthlyBudgetsState || {}).length;
                window.updateSyncDebug(`Fetch 200 OK: ${txCount} tx, ${budgetCount} months`);
            }
        } else {
            console.warn('[FinanceOS] Server returned status: ' + res.status);
            if (window.updateSyncDebug) window.updateSyncDebug('Fetch failed: HTTP ' + res.status);
        }
    } catch (e) {
        console.warn('[FinanceOS] Server unreachable or timed out (' + (e.name || 'Error') + '). Fallback to local storage.');
        if (window.updateSyncDebug) window.updateSyncDebug('Fetch error: ' + (e.name || 'Error') + ' ' + (e.message || ''));
    }

    // 2. Load from localStorage if server failed or is empty
    let localData = null;
    try {
        const localDataRaw = localStorage.getItem('financeOS_master_data');
        localData = localDataRaw ? JSON.parse(localDataRaw) : null;
        if (window.updateSyncDebug) window.updateSyncDebug('Local data parsed successfully');
    } catch (e) {
        console.error('[FinanceOS] Failed to parse local data', e);
        if (window.updateSyncDebug) window.updateSyncDebug('Local data parse error: ' + e.message);
    }

    // 3. Smart Merge Strategy
    let data = remoteData || localData;
    ...
    if (remoteData && localData) {
        ...
        if (localTs > remoteTs) {
            console.log("[FinanceOS] LocalStorage is newer. Merging remote transactions into local.");
            // Merge transactions (simple union by ID if possible, but here we just prefer newer state)
            data = localData;
        } else {
            console.log("[FinanceOS] Remote data is newer or equal. Merging local budget months if missing.");
            // Deep merge budget months: don't lose local months not on server
            if (localData.monthlyBudgetsState && remoteData.monthlyBudgetsState) {
                Object.keys(localData.monthlyBudgetsState).forEach(month => {
                    if (!remoteData.monthlyBudgetsState[month]) {
                        remoteData.monthlyBudgetsState[month] = localData.monthlyBudgetsState[month];
                    }
                });
            }
            data = remoteData;
    ...
    if (data) {
        if (data.monthlyBudgetsState && Object.keys(data.monthlyBudgetsState).length > 0) {
            monthlyBudgetsState = data.monthlyBudgetsState;
        } else if (data.budgetState) {
            // Legacy budgetState found, migrate to current view
            monthlyBudgetsState[currentMonthView] = data.budgetState;
        }
