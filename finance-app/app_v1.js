...
const appActions = {
    ...
    'force_restore': async () => {
        if (confirm('모든 데이터를 백업본으로 강제 복구하시겠습니까? (현재 데이터가 덮어씌워질 수 있습니다)')) {
            console.log('[FinanceOS] Force Restoration Triggered...');
...
async function loadData(forceRescue = false) {
    if (!currentUser) return false;
    console.log("[FinanceOS] loadData starting (forceRescue=" + forceRescue + ")...");
    ...
    try {
        ...
        console.log("[FinanceOS] loadData complete.");
        return true;
    } catch (e) {
        console.error('[FinanceOS] FATAL LOAD ERROR:', e);
        return false;
    }
}
